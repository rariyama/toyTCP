...
pub struct Socket {
    ...
    pub connected_connection_queue: VecDeque<SockID>,  // the queue containing connected socket
...
pub struct SendParam {
    pub unacked_seq: u32, // the head of seq which is not acked yet.
    pub next: u32,        // next send 
    pub window: u16,      // window size
    pub initial_seq: u32, // seq which is initialy sent
}

#[derive(Clone, Debug)]
pub struct RecvParam {
    pub next: u32,        // the seq which will be received next.
    pub window: u16,      // receive window
    pub initial_seq: u32, // seq which is initialy sent
    pub tail: u32,        // the tail of received seq.
}
...
impl Socket {
    ...
    pub fn send_tcp_packet(&mut self, seq: u32, ack: u32, flag: u8, payload: &[u8]) -> Result<usize> {
        ...
        tcp_packet.set_data_offset(5); // set constant value because of not using option field.
