...
use pnet::packet::{Packet, ip::IpNextHeaderProtocols::{self, Tcp}, tcp::TcpPacket};
...
impl TCP {
    pub fn new() -> Arc<Self> {
        ...
        std::thread::spawn(move || {
            // thread for receving packet.
            cloned_tcp.receive_handler().unwrap();
        });
        let cloned_tcp = tcp.clone();
        std::thread::spawn(move || {
            //timer sthead for managing retransmission
            cloned_tcp.timer();
        });
        tcp
    }
        ...
        loop {
            ...
            for (sock_id, socket) in table.iter_mut() {
                while let Some(mut item) = socket.retransmission_queue.pop_front() {
                    // remove segments acked from retransmission queue
    ...
    pub fn recv(&self, sock_id: SockID, buffer: &mut [u8]) -> Result<usize> {
        ...
        println!("{}", socket.recv_buffer.len());
        println!("{}", socket.recv_param.window);        
    ...
    // issue events to the designated socket
    ...
    fn close_handler(&self, socket: &mut Socket, packet: &TCPPacket) -> Result<()> {
        ...
        socket.send_param.unacked_seq = packet.get_seq();
        Ok(())
    }
    ...
    fn receive_handler(&self) -> Result<()> {
        ...
        loop {
            ...
            // transform tcp::TCPPacekt from pnet
    ...
    // remove acked segment if it can be successfully retransmitted.
    ...
    // process packet arrives establish socket.
    ...
    fn process_payload(&self, socket: &mut Socket, packet: &TCPPacket) -> Result<()> {
        ...
        if copy_size > 0 {
            socket.send_tcp_packet(socket.send_param.next, 
                                   socket.recv_param.next,
                                   tcpflags::ACK,
                                &[],
                                   )?;
    ...
    // to process packets which arrive at LISTEN socket.
    fn listen_handler(
        &self, 
        ...
        if packet.get_flag() & tcpflags::SYN > 0 {
            // process passive open
            // generate new socket which will become connected sockets later
    ...
    // to process packets which arrive at SYNRCVD socket
    fn synrcvd_handler(
        &self,
        mut table: RwLockWriteGuard<HashMap<SockID, Socket>>,
        sock_id: SockID,
          ...
          && packet.get_ack() <= socket.send_param.next
        {
            ...
            socket.send_param.unacked_seq = packet.get_ack();
            socket.status = TcpStatus::Established;
            dbg!("stauts: synrcvd ->", &socket.status);
...
// generate listening socket and return socket id
...
// wait until generating connected socket and return id after completing.
